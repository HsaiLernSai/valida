  ...
  return (
    <Card as="section" className="p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {popularHashtags.map((tag) => <a href={discoveryUrl({ hashtag: tag })} key={tag} className="inline-flex min-h-9 items-center rounded-lg bg-surface px-2.5 py-1.5 text-[11px] font-semibold text-slate-600 transition hover:bg-brand-soft hover:text-brand-dark focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/15">#{tag}</a>)}
      </div>
