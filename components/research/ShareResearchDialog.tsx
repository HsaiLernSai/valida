...
export function ShareResearchDialog({ post, onClose, onShared }: ShareResearchDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-end justify-center bg-ink/45 p-0 backdrop-blur-sm sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={`share-title-${post.id}`} aria-describedby={`share-description-${post.id}`} className="safe-area-footer flex max-h-[94dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-3xl border border-white/70 bg-white shadow-floating sm:rounded-3xl">
          ...
          {isPublic ? (
            <div className="grid gap-5 sm:grid-cols-[minmax(0,1fr)_190px]">
              <div>
                ...
                <div className="mt-2 flex gap-2"><input id={`share-url-${post.id}`} readOnly value={shareUrl} className="min-h-11 min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-600 outline-none focus:border-brand/50 focus:ring-4 focus:ring-brand/10" /><Button type="button" onClick={copyLink} className="min-h-11 shrink-0 rounded-xl px-4 text-xs">Copy Link</Button></div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-center">
                {qrMatrix ? <svg viewBox={`0 0 ${qrMatrix.length + 8} ${qrMatrix.length + 8}`} role="img" aria-label={`QR code for ${post.title}`} className="mx-auto aspect-square w-full rounded-lg bg-white shadow-sm" shapeRendering="crispEdges"><rect width="100%" height="100%" fill="white" />{qrMatrix.map((row, y) => row.map((dark, x) => dark ? <rect key={`${x}-${y}`} x={x + 4} y={y + 4} width="1" height="1" fill="#172033" /> : null))}</svg> : <div className="grid aspect-square place-items-center rounded-lg bg-white text-xs text-rose-600">QR unavailable</div>}
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-6 text-center"><div className="mx-auto grid h-11 w-11 place-items-center rounded-2xl bg-white text-lg font-black text-amber-700 shadow-sm">!</div><h3 className="mt-3 text-base font-black text-ink">Public sharing is not available yet</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-600">This research exists only in your current browser session. Public links and QR codes require future server persistence, which is outside this sprint.</p><Button type="button" variant="secondary" onClick={onClose} className="mt-4 min-h-11 rounded-xl px-4 text-xs">Close</Button></div>
          )}
        </div>
      </div>
    </div>
