  ...
  return (
    <div>
      ...
      <div className="mt-5 flex gap-2">
        ...
        <button type="button" onClick={() => addHashtag(input)} className="rounded-xl border border-slate-200 bg-surface px-4 text-xs font-bold text-ink hover:bg-surface">Add</button>
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 flex flex-wrap gap-2">{suggestedHashtags.map((tag) => <button type="button" key={tag} onClick={() => addHashtag(tag)} disabled={data.hashtags.includes(tag)} className="rounded-full border border-slate-200 bg-surface px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:border-brand/30 disabled:opacity-40">#{tag}</button>)}</div>
      </div>
    </div>
  );
}
