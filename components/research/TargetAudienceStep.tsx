    ...
    <div>
      ...
      <div className="mt-5">
        ...
        <div className="mt-1.5 flex gap-2">
          <input id="audience-input" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === ",") { event.preventDefault(); addAudience(input); } }} placeholder="Add an audience" className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-ink outline-none transition placeholder:text-slate-400 focus:border-brand/50 focus:ring-4 focus:ring-brand/5" />
          <button type="button" onClick={() => addAudience(input)} className="rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-ink transition hover:bg-slate-50">Add</button>
        </div>
