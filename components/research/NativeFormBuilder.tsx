...
const fieldClass = "w-full rounded-xl border border-slate-200 bg-surface px-3.5 py-3 text-sm text-ink shadow-sm outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-brand/60 focus:ring-4 focus:ring-brand/10";
...
export function NativeFormBuilder({ questions, onChange }: { questions: NativeFormQuestion[]; onChange: (questions: NativeFormQuestion[]) => void }) {
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-surface shadow-[0_12px_34px_rgba(50,65,110,0.05)]">
      <div className="border-b border-slate-200 bg-gradient-to-br from-surface via-surface to-blue-50/70 px-4 py-4 sm:px-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <span className="rounded-full bg-surface px-3 py-1.5 text-[11px] font-extrabold text-brand-dark ring-1 ring-blue-100">{questions.length} {questions.length === 1 ? "question" : "questions"}</span>
        </div>
      </div>
      ...
      <div className="space-y-3.5 p-3 sm:p-4">
        {questions.map((question, questionIndex) => {
          ...
          return (
            <section key={question.id} aria-labelledby={`${question.id}-heading`} className={`overflow-hidden rounded-2xl border bg-surface shadow-sm transition ${needsPrompt || hasInvalidOption ? "border-amber-200" : "border-slate-200"}`}>
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 bg-surface px-3.5 py-3 sm:px-4">
              ...
              <div className="space-y-4 p-3.5 sm:p-4">
                ...
                {optionQuestion && (
                  <div className="rounded-2xl border border-slate-200 bg-surface p-3 sm:p-3.5">
                      ...
                      {(question.options ?? []).map((option, optionIndex) => {
                        ...
                        return (
                          <div key={`${question.id}-${optionIndex}`} className="flex items-center gap-2">
                            <span className={`grid h-8 w-8 shrink-0 place-items-center border bg-surface text-[10px] font-bold text-slate-400 ${question.type === "multiple_choice" ? "rounded-full" : "rounded-lg"}`}>{question.type === "dropdown" ? "⌄" : optionIndex + 1}</span>
                            <input id={`${question.id}-option-${optionIndex}`} aria-label={`Question ${questionIndex + 1} option ${optionIndex + 1}`} value={option} onChange={(event) => updateQuestion(question.id, { options: (question.options ?? []).map((item, index) => index === optionIndex ? event.target.value : item) })} placeholder={`Option ${optionIndex + 1}`} aria-invalid={optionInvalid || hasDuplicateOption} className={`min-h-10 min-w-0 flex-1 rounded-lg border bg-surface px-3 text-xs text-ink outline-none transition focus:ring-4 ${optionInvalid || hasDuplicateOption ? "border-amber-300 focus:border-amber-400 focus:ring-amber-100" : "border-slate-200 focus:border-brand/50 focus:ring-brand/10"}`} />
                    ...
                    <button type="button" onClick={() => addOption(question)} className="mt-3 min-h-10 rounded-lg border border-dashed border-brand/30 bg-surface px-3 text-[11px] font-extrabold text-brand transition hover:border-brand/50 hover:bg-brand-soft focus:outline-none focus:ring-4 focus:ring-brand/10">+ Add option</button>
                  </div>
                )}

                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
                  <p className="text-[10px] leading-4 text-slate-400">Required questions must be answered before submission.</p>
                  <label className="flex min-h-10 cursor-pointer items-center gap-2.5 rounded-xl border border-slate-200 bg-surface px-3 text-[11px] font-extrabold text-slate-600 transition hover:border-brand/25">
      ...
      <div className="border-t border-slate-200 bg-surface px-3.5 py-4 sm:px-5">
