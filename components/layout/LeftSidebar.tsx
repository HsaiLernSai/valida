...
export function LeftSidebar({ onCreateResearch }: { onCreateResearch?: () => void }) {
  ...
  const navigationSections = [
    ...
    { label: user ? "My Space" : "Account", items: user ? navigationItems.slice(3) : guestItems },
  ];
          ...
          <div key={section.label}>
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => (
                <a key={item.label} href={item.href} aria-current={isActive(item.href) ? "page" : undefined} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${isActive(item.href) ? "bg-brand-soft text-brand-dark" : "text-slate-600 hover:bg-white hover:text-ink"}`}>
