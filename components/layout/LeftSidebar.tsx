...
export function LeftSidebar({ onCreateResearch }: { onCreateResearch: () => void }) {
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-[224px] shrink-0 flex-col border-r border-slate-200/70 bg-white/70 px-4 py-5 backdrop-blur-xl lg:flex">
          ...
          <div key={section.label}>
            ...
            <div className="space-y-0.5">
              {section.items.map((item, itemIndex) => (
                <a key={item.label} href={item.href} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${sectionIndex === 0 && itemIndex === 0 ? "bg-brand-soft text-brand-dark" : "text-slate-600 hover:bg-white hover:text-ink"}`}>
      ...
      <Button onClick={onCreateResearch} className="mt-5 gap-2 rounded-xl px-4 py-2.5 text-sm"><Icon name="plus" className="h-4 w-4" /> Start research</Button>
